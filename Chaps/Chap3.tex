...
那么\autoref{3.114}成为
\begin{align}
	\left[ \int\dd\omega_1\alpha^*(\omega_1)f(\mathbf{x}_1)\alpha(\omega_1) \right]\psi_j(\mathbf{r}_1) &= 
	% 注：尽管原书单电子算符写为 $h(\mathbf{r}_1)$，但是结合上下文应为 $h(\mathbf{x}_1)$，此处应为原书笔误
	\left[ \int\dd\omega_1\alpha^*(\omega_1)h(\mathbf{x}_1)\alpha(\omega_1) \right]\psi_j(\mathbf{r}_1) \notag\\
